...
<?php
    ...
    function errorAlert($alert){
        echo "<div class='errorAlert'>{$alert}<a href='signUp.php'>회원가입 다시하기</a><a href='logIn.php'>로그인하기</a></div>";
        return;
    }
    ...
    if($userName == null || $userName == ''){
        errorAlert("이름을 입력해 주세요!");
        exit;
    }
    if($userNickName == null || $userNickName == ''){
        errorAlert("닉네임을 입력해 주세요!");
        exit;
    }
    if($userPW == null || $userPW == ''){
        errorAlert("패스워드를 입력해 주세요!");
        exit;
    }
    if($birthYear == 0){
        errorAlert("생일 년도를 입력해 주세요!");
        exit;
    }
    if($birthMonth == 0){
        errorAlert("생일 월을 입력해 주세요!");
        exit;
    }
    if($birthDay == 0){
        errorAlert("생일 일을 입력해 주세요!");
        exit;
    }
    ...
    if($result){
        ...
        if($count == 0){
            ...
            errorAlert("이메일이 중복됩니다. 다시 확인해 주세요.");
            exit;
        }
    }else{
        errorAlert("에러 발생 : 관리자에게 문의하세요(1)");
        exit;
    }
    ...
    if($result){
        ...
        if($count == 0){
            ...
            errorAlert("닉네임이 중복됩니다. 다시 확인해 주세요.");
            exit;
        }
    }else{
        errorAlert("에러 발생 : 관리자에게 문의하세요(2)");
        exit;
    }

    // 위의 조건이 이상이 없으면 데이터 입력
    if( $userEmailCheck = true && $userNickNameCheck = true ){
        ...
        if($result){
            ...
            errorAlert("에러 발생 : 관리자에게 문의하세요(3)");
            exit;
        }
    }else{
